...
k4_bams = bams[grepl("01562", bams) & grepl("K4", bams)]
k4_bws = bws[grepl("01562", bws) & grepl("K4", bws)]
k27_bams = bams[grepl("01562", bams) & grepl("K4", bams)]
k27_bws = bws[grepl("01562", bws) & grepl("K27", bws)]
k4_peaks = k4_peaks[grepl("01562", names(k4_peaks)) & grepl("K4", names(k4_peaks))]
k27_peaks = k27_peaks[grepl("01562", names(k27_peaks)) & grepl("K27", names(k27_peaks))]

k4_peaks = easyLoad_narrowPeak(k4_peaks)
k27_peaks = easyLoad_narrowPeak(k27_peaks)

biv_olaps = ssvOverlapIntervalSets(append(k4_peaks, k27_peaks))


ssvFeatureBinaryHeatmap(biv_olaps)

hist(width(biv_olaps), breaks = 200)
biv_dt = ssvFetchBigwig(c(k4_bws, k27_bws), resize(biv_olaps, 20000, fix = "center"), 
                        return_data.table = TRUE, win_size = 50, 
                        win_method = "summary", summary_FUN = weighted.mean)
cbiv_dt = centerAtMax(biv_dt, view_size = .1, by_ = c("id"))


biv_clust = ssvSignalClustering(cbiv_dt, max_rows = Inf)
biv_clust_assignment = unique(biv_clust[, .(id, cluster_id)])
clust_memb = unique(biv_clust[, .(id, cluster_id)])[, .(memb_id = list(id)), by = .(cluster_id)][order(cluster_id)]
pcall = t(sapply(clust_memb$memb_id, function(x){
    clust_olap = biv_olaps[names(biv_olaps) %in% x]
    colSums(as.data.frame(mcols(clust_olap))) / length(x)
}))
round(pcall, 2)
ssvSignalHeatmap(biv_clust[id %in% sample(unique(id), 500)]) + labs(title = "FE")
