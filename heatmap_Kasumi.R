...
k4_gr.raw = easyLoad_narrowPeak((c(k4_peaks[grepl("Kasum", names(k4_peaks))], 
                                   k27_peaks[grepl("Kasum", names(k27_peaks))])))
k4_gr = lapply(k4_gr.raw, function(x)subset(x, qValue > 10))
# k4_olap = ssvOverlapIntervalSets(k4_gr)
k4_olap = subsetByOverlaps(k4_gr[[1]], k4_gr[[2]])
...
k4_olap = resize(k4_olap, 16000, fix = "center")
...
tmp_fudge = 1e6
bw_dt[grepl("K27me", sample), y := y / tmp_fudge]
...
names(k4_olap) = k4_olap$name
biv = names(k4_olap)

clust_dt = ssvSignalClustering(bw_dt[id %in% biv], nclust = 6, max_rows = Inf, max_cols = Inf)
...
odir = "k4_subset_Kasumi_16k"
dir.create(odir)
setwd(odir)

ssvSignalHeatmap(clust_dt[mark == "H3K27me3" & id %in% id_k], fill_ = "ycap", facet_ = "cell") + labs(title = "H3K27me3")
ggsave("hmap_k27.pdf", width = 12, height = 8)
ssvSignalHeatmap(clust_dt[mark == "H3K4me3" & id %in% id_k], fill_ = "ycap", facet_ = "cell") + labs(title = "H3K4me3")
ggsave("hmap_k4.pdf", width = 12, height = 8)

ssvSignalHeatmap(clust_dt[mark == "H3K27me3" & id %in% id_k], fill_ = "y", facet_ = "cell") + labs(title = "H3K27me3")
ggsave("hmap_k27_raw.pdf", width = 12, height = 8)
ssvSignalHeatmap(clust_dt[mark == "H3K4me3" & id %in% id_k], fill_ = "y", facet_ = "cell") + labs(title = "H3K4me3")
...
anno_dt = my_annotate(GRanges(cluster_assignment))
...
bg_dt = my_annotate(k4_gr[grepl("K4", names(k4_gr))])
...
pgo
ggsave("go_res.pdf", height = 30, width = 10)
